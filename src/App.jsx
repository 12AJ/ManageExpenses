...
// for filtered year coming from user
    ...
    function fromFilterSearch(searchInput){
        setFilterSearch(searchInput.toLowerCase())
    }
...
//to filter the list acording to month selected by user
    ...
    //to filter the list acording to day selected by user
    let filterDt = filterMnth.filter((data)=>{
      ...
      //to filter the list acording to search selected by user
let filtersrh = filterDt.filter((data)=>{
    if(filterSearch ==""){
        return data;
    }
    else{
        
        return data.item.toLowerCase().includes(filterSearch)
    }
})
 ...
 sortBy(filtersrh);
    ...
    return (
        <div className="container">
            ...
            <div className="main">
                <FilterDateList fromFilterYear={fromFilterYear} fromFilterMonth={fromFilterMonth} fromFilterDate={fromFilterDate} fromFilterSearch={fromFilterSearch}/>
                <Graph filteryr = {filteryr} />
                {
                    filtersrh == 0  && <h3 className="nodataf">No Data Found</h3>
                }
                {
                  filtersrh.map(list => {
                            return (
